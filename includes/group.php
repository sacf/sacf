...
class group {
	...
	private function make() {
		...
		foreach($this->fields as $field) {
			$arr['fields'][] = $field->make_slug($this->name)->make();
        }
    ...
    public function export() {
        ...
        var_dump('---------');
        var_dump('group: ' . $groups['slug'] . ' : ' . $groups['key']);
        foreach ($groups['fields'] as $field) {
            var_dump('field: ' . $field['slug'] . ' : ' . $field['key']);
            if (isset($field['sub_fields'])) {
                foreach ($field['sub_fields'] as $subfields) {
                    var_dump(' + sf: ' . $subfields['slug'] . ' : ' . $field['key']);
                }
            }
            if (isset($field['layouts'])) {
                foreach ($field['layouts'] as $layout) {
                    foreach ($layout['sub_fields'] as $subfields) {
                        var_dump(' + fc: ' . $subfields['slug'] . ' : ' . $field['key']);
                    }
                }
            }
        }
